...
``` r
instacart %>% 
  ggplot(aes(x = department, y = order_number)) + geom_boxplot()
```
...
``` r
instacart %>% 
  ggplot(aes(x = department, y = days_since_prior_order)) + geom_point()
```
...
<img src="p8105_hw3_th2891_files/figure-gfm/unnamed-chunk-4-1.png" width="90%" />
...
accel %>% 
  ...
  pivot_longer(
    ...
    names_to = "time",
    values_to = "activity") %>% 
  ggplot(aes(x = time , y = activity)) + geom_point(aes(color = day))
```

<img src="p8105_hw3_th2891_files/figure-gfm/unnamed-chunk-12-1.png" width="90%" />
